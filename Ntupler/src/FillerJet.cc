...

{
    ...
    // For PUID
    fPUID  = iC.consumes<edm::ValueMap<int> >(iConfig.getParameter<edm::InputTag>("jetsPUIDPF"));
    fPUMVA = iC.consumes<edm::ValueMap<float> >(iConfig.getParameter<edm::InputTag>("jetsPUIDMVAPF"));
        ...
        const pat::TriggerObjectStandAloneCollection *patTriggerObjects)
{
    ...
    for(reco::PFJetCollection::const_iterator itJet = jetCol->begin(); itJet!=jetCol->end(); ++itJet) {
        ...
        if(passLoose) {
            ...
            pJet->mva = fJetPUIDMVACalc.mvaValue((float)pvCol->size(), 
                                                 ptRaw*jetcorr, itJet->eta(), itJet->phi(),
                                                 pJet->d0, pJet->dz, 
                                                 pJet->beta, pJet->betaStar, 
                                                 itJet->chargedMultiplicity(), itJet->neutralMultiplicity(),
                                                 dRMean, pJet->dR2Mean, 
                                                 pJet->ptD, frac01, frac02, frac03, frac04, frac05);
